/**
 * @file test_bignum_abs_benchmark_adapter.c
 * @brief Deterministic tests for the bignum_abs benchmark adapter.
...
static void test_validation(void)
{
    ...
    assert(bignum_abs_benchmark_validate_workload(&workload) ==
        BIGNUM_ABS_BENCHMARK_STATUS_SUCCESS);
    invalid.operation_kind = "xor";
    assert(bignum_abs_benchmark_validate_workload(&invalid) ==
        BIGNUM_ABS_BENCHMARK_STATUS_INVALID_PROFILE);
    assert(bignum_abs_benchmark_validate_workload(NULL) ==
        BIGNUM_ABS_BENCHMARK_STATUS_NULL_ARGUMENT);
}
...
static void test_callbacks(void)
{
    ...
    assert(bignum_abs_benchmark_adapter_init(NULL) ==
        BIGNUM_ABS_BENCHMARK_STATUS_NULL_ARGUMENT);
    assert(bignum_abs_benchmark_adapter_init(&adapter) ==
        BIGNUM_ABS_BENCHMARK_STATUS_SUCCESS);
...
int main(void)
{
    ...
    test_legacy_modes();
    test_callbacks();
    puts("bignum_abs benchmark adapter tests: OK");
    return EXIT_SUCCESS;
}
